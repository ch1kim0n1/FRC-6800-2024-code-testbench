...
//first initiation, numbers are changable
//double tx=frc::SmartDashboard::GetNumber("tx",0);
//double ty=frc::SmartDashboard::GetNumber("ty",0);
//use these from the network tables to put it in the shufflebaord!!! yw!!
...
{
    //Created two LL variables to cover both cameras at the same time
    limeTableThree = nt::NetworkTableInstance::GetDefault().GetTable("limelight-three");
    limeTableTwo = nt::NetworkTableInstance::GetDefault().GetTable("limelight-two");
...
{
    // Get tx and ty from the Limelight3
    double txTH = limeTableThree->GetNumber("tx",0.0);
    double tyTH = limeTableThree->GetNumber("ty",0.0);
    // Get tx and ty from the Limelight2
    double txTW = limeTableTwo->GetNumber("tx",0.0);
    double tyTW = limeTableTwo->GetNumber("ty",0.0);

    // Put tx and ty on the Shuffleboard3
    frc::SmartDashboard::PutNumber("LimelightThree/tx", txTH);
    frc::SmartDashboard::PutNumber("LimelightThree/ty", tyTH);
    // Put tx and ty on the Shuffleboard2
    frc::SmartDashboard::PutNumber("LimelightTwo/tx", txTW);
    frc::SmartDashboard::PutNumber("LimelightTwo/ty", tyTW);
    ...
    //LL 2 logic is added
    if (limeTableTwo->GetNumber("tv", 0) == 1.0) {
        
        std::vector<double> poseArray;
        if (frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kBlue) {
            poseArray = limeTableTwo->GetNumberArray("botpose_wpiblue", std::span<const double>());
        } else {
            poseArray = limeTableTwo->GetNumberArray("botpose_wpired", std::span<const double>());
        }
    ...
    //LL 3 logic is added
    if (limeTableThree->GetNumber("tv", 0) == 1.0) {
        
        std::vector<double> poseArray;
        if (frc::DriverStation::GetAlliance() == frc::DriverStation::Alliance::kBlue) {
            poseArray = limeTableThree->GetNumberArray("botpose_wpiblue", std::span<const double>());
        } else {
            poseArray = limeTableThree->GetNumberArray("botpose_wpired", std::span<const double>());
        }
...
{    
    ...
    if (state.xPose)
    {
        ...
        limeTableTwo->PutNumber("pipeline", LimelightPipes::APRIL_TAGS);
        limeTableThree->PutNumber("pipeline", LimelightPipes::APRIL_TAGS);    
